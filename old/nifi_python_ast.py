...
docker_cmd_eng=""
docker_rep="1"
data_dir="/home/ubuntu/linear_data"	# Result from data management layer
data_dir_container=""
...
for request in request_in:
	...
	if os.path.exists(os.path.join(script_path_in,request,conf_file_name)):
		...
		f_log.write("Configuration does not exsit for "+request+" at "+time.strftime("%c")+"\n")
                continue
	...
	if not check_input_files(os.path.join(script_path_in,request),conf_in[total_inputs_key]):
		f_log.write("Some or all inputs files (folders) are missing for "+request+" at "+time.strftime("%c")+"\n\n")
		continue
...
#			os.remove(os.path.join(script_processed,general_python_script_name))	# This step cannot be done before the "if condition". Otherwise, the file will be removed before docker task finishes
			...
			f_log.write("FINISHED at "+time.strftime("%c")+"\n\n")
...
#			os.remove(os.path.join(script_processed,general_python_script_name))	# This step cannot be done before the "if condition". Otherwise, the file will be removed before docker task finishes
			...
			f_log.write("FAILED at "+time.strftime("%c")+"\n\n")
